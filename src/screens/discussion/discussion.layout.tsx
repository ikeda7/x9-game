...
export function TimerPanel({ mm, ss, done, warn, running, toggle }: TimerPanelProps) {
  return (
    <div className={`discussion-timer${warn ? ' discussion-timer--warn' : ''}`}>
      <div className={`discussion-timer-digits${warn ? ' discussion-timer-digits--warn' : ''}`}>
        {mm}:{ss}
      </div>
  ...
  return (
    <div className="discussion-starter-badge">
      ...
      <span className="x9-small discussion-starter-text">
        Começa falando:{' '}
        <span className="discussion-starter-name">{name}</span>
      </span>
    </div>
  )
}
  ...
  return (
    <div className="discussion-player-list">
      {order.map((p, i) => {
        const isStarter = i === 0
        return (
          <div key={p.id} className={`discussion-player-row${isStarter ? ' discussion-player-row--starter' : ''}`}>
            <span className="discussion-player-badge">{String(i + 1).padStart(2, '0')}</span>
            <span className="discussion-player-name">{p.name}</span>
            {isStarter && <span className="x9-label discussion-starter-tag">Começa</span>}
          </div>
        )
      })}
      ...
      {eliminated.length > 0 && (
        <>
          <div className="x9-label discussion-eliminated-header">Fora ({eliminated.length})</div>
          {eliminated.map((p) => (
