...
export default function VotingScreen(props: Props) {
  return props.mode === 'secret' ? <SecretVoting {...props} /> : <OpenVoting {...props} />
}
...
function stepBtn(disabled: boolean, accent: boolean) {
  return {
    ...
    background: disabled ? 'transparent' : accent ? 'rgba(176,38,255,0.12)' : 'var(--bg-elevated)',
      ...
      <Screen dark>
        ...
        >
          <div className="x9-mono" style={{ color: 'var(--text-low)', fontSize: 13 }}>
            {String(voterIndex + 1).padStart(2, '0')} / {String(alive.length).padStart(2, '0')}
          </div>
          ...
          <p className="x9-small" style={{ marginTop: 16, color: 'var(--text-low)', maxWidth: 250 }}>
            Que ninguém veja seu voto. Quando estiver com {voter.name}, toque para votar.
          </p>
  ...
  return (
    <Screen dark>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '34px 22px 24px' }}>
        ...
        <div style={{ marginTop: 20, flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 11 }}>
          {suspects.map((p) => {
